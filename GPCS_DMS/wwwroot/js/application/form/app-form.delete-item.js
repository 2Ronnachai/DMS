class DeleteItemForm{
    ...
    async render() {
        try{
            ...
            console.error('Error rendering DeleteItemForm:', error);
            this.appForm.appMain.notification.error('Failed to load the form. Please try again later.');
        }
    }
    ...
    _renderForm(){
        ...
        formContainer.className = 'delete-item-form';
        ...
        this._renderDeleteItemsForm();
        this._renderDeleteItemsGrid();
        this._renderButtons();
    }
    ...
    async _loadItems() {
        ...
        try {
            const items = await this.appForm.appMain.formComponents.getItemsDataSourceWithGPCSDetails(payload);
    ...
    _renderDeleteItemsGrid() {
        const gridConfig = {
            ...
            columns: [
                ...
                { dataField: 'po', caption: 'PO', visible: true, fixed: true, fixedPosition: 'right'},
                { dataField: 'pr', caption: 'PR', visible: true, fixed: true, fixedPosition: 'right'},
                { dataField: 'inventory', caption: 'Inv' ,visible: true, fixed: true, fixedPosition: 'right'}
            ],
            onCellPrepared: (e) => {
                if (e.rowType === 'data' && 
                ...
                // Disable selection for rows with po > 0 or pr > 0 or inventory > 0
                if (e.rowType === 'data' && e.column.command === 'select') {
                    const rowData = e.row.data;
                    if (rowData.po > 0 || rowData.pr > 0 || rowData.inventory > 0) {
                        e.cellElement.find('.dx-select-checkbox').dxCheckBox('instance').option('disabled', true);
                        e.cellElement.addClass('disabled-row');
                    }
                }
            },
            ...
            onSelectionChanged: (e) => {
                ...
                const invalidRows = selectedRows.filter(row => 
                    (row.po > 0) || (row.pr > 0) || (row.inventory > 0)
                );
                ...
                if (invalidRows.length > 0) {
                    ...
                    this.appForm.appMain.notification.warning('Cannot select items that have PO, PR, or Inventory greater than 0');
                }
            }
        };
