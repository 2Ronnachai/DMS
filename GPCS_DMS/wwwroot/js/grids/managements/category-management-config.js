async function createCategoryManagementGridConfig() {
    let costCenters = [];
    try {
        const response = await Http.getCache('purchaserCostCenter/lookups/', 10 * 60 * 1000);
...
window.createCategoryManagementGridConfig = createCategoryManagementGridConfig;
